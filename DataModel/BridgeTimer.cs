    ...
    public partial class BridgeTimer : TimerSetting
    {
        ...
        #region Public Methods
            public async void OpenSetting()
            {
                ...
                WarningTime    = _fiveMinutes.Subtract(_transitionTime);
            ...
            {
                if (!_isPaused)
                    if (_remainingTime == WarningTime)
                        AudioPlayer.Play(Sound, Volume);                            // WarningTime in Round
                    else
                        if (_isAtBreak && _remainingTime == _twoMinutes)
                            AudioPlayer.Play(Sound, Volume);                        // WarningTime at Break
                        else
                            if (_remainingTime == TimeSpan.Zero)
                                if (!_isAtBreak && _round == BreakAfter)
                                {
                ...
                if (_isAtBreak)
                    ...
                    WarningVisiblity = (_remainingTime >  TimeSpan.Zero && _remainingTime <  WarningTime) ? Visibility.Visible : Visibility.Collapsed;
                ...
                if (_round == Rounds)
                {
                    ...
                    if (_round <  Rounds)
                        ...
                        {
                            if (_round <= BreakAfter)
                                MoreInfo = $"Pause efter {BreakAfter}. {RoundText}";

                            if (_isAtTransition)
                                Round = $"Der skiftes til {_round}. {RoundText}";
                            else
